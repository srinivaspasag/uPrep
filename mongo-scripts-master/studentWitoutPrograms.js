db.orgmembers.find({orgId:"558400c4e4b0f968650c7a13","profile":"STUDENT","mappings":{$exists:false}}).sort({"timeCreated":1}).forEach(function(doc){
    ...
    try{
        var programs = "";
        if(typeof doc.expiredMappings[0] !== 'undefined' && doc.expiredMappings[0] !== 'null'){
            if(doc.expiredMappings.length > 0){
                for(var i=0; i < doc.expiredMappings.length; i++){
                    var objectId = new ObjectId(doc.expiredMappings[i].programId);
                    db.orgprograms.find({"_id":objectId}).forEach(function(program){
                        if(i != 0) {
                            programs+=" & ";
                        }
                        programs+=program.name +" Added Time- "+new Date(doc.expiredMappings[i].timeJoined)+" End Time- "+new Date(doc.expiredMappings[i].endTime);
                    });
                }
            }
        }else{

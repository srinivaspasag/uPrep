...
public class AssignmentExporter extends AbstractContentExporter {

    public static AssignmentExporter INSTANCE = new AssignmentExporter();
    private static final ALogger     LOGGER   = Logger.of(AssignmentExporter.class);
    ...
    public boolean export(ExportRecordManager manager, EntityExportRecord entityRecord,
        ...
        try {
            ...
            Assignment globalTest = (Assignment) verifyPublishing(entityRecord.content);

            SrcEntity globalAssignmentEntity = new SrcEntity(EntityType.ASSIGNMENT,
            ...
            try {
                resource = ContentManager.getContentLink(record.contentSrc.id, record.userId,
                        globalAssignmentEntity, target, UserActionType.ADDED,
                        VedantuRecordState.ACTIVE);
            } catch (VedantuException e1) {

                throw new ExportException(VedantuErrorCode.FAILED_TO_EXPORT, e1);
            }
            ...
            Assignment globalAssignment = AssignmentDAO.INSTANCE.getById(globalAssignmentEntity.id);
            if (CollectionUtils.isNotEmpty(globalAssignment.__getAllQIds())) {
                for (String questionId : globalAssignment.__getAllQIds()) {
                    ...
                    EntityExportRecord questionRecord = new EntityExportRecord(new SrcEntity(
                            EntityType.QUESTION, questionId), globalAssignment.timeCreated);

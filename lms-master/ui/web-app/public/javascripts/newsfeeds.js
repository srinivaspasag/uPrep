var newsEntityProcessor = new function(){
	...
	this.process = function(feedLocation,feed,actionType,clustered){
		...
		switch(feedLocation){
			case "NOTIFICATIONS" : html = notiInsert(textHtml,feed);
					break;
	...
	var getSrcEntity = function(src,feed,clickable){
		switch(src.type){
			...
			case "TEST" : return getTest(src,feed,clickable);
			...
			case "FILE" : return getFile(src,clickable); 
                        case "MODULE" : return getModule(src,feed,clickable);
	...
	var getDoubt = function(src,clickable){
		...
		var aTag = i18nJS("ENTITY_DOUBT")+" - \"<a class='openInstDoubtExt doCStream' data-diss-id='"+src.id+"' href='"+openUrl+"'>"+name+"</a>\"";
		return aTag;
	};
	var getTest = function(test,feed,clickable){
		var orgId = $("#myInstitutePage").data('orgId');
		...
		if(orgId){
				...
				if(userRole == "MANAGER" || userRole == "TEACHER"){
					data += "<a class='"+className+"' data-test-id='"+test.id+"' data-org-id='"+orgId+"'\n\
                            href='/organization/"+orgId+"/"+testUrl+"/"+test.id+"'>"+test.name+"</a>";
				}
				else if(userRole == "STUDENT"){
					if(target == null || target == undefined || target == ""){
						data += "<span data-test-id='"+test.id+"' title='Please go to the tests section of library to take test.' >"+test.name+"</span>";
					}
					else{
						data += "<a class='"+className+"' data-test-id='"+test.id+"' data-target-id='"+target.id+"' data-target-type='"+target.type+"' data-org-id='"+orgId+"'\n\
	                            href='/organization/"+orgId+"/"+testUrl+"/"+test.id+"?targetId="+target.id+"&targetType="+target.type+"'>"+test.name+"</a>";
					}
				}
				else{
					data += "<span data-test-id='"+test.id+"' title='Please go to the tests section of library to take test.' >"+test.name+"</span>";
	...
	var getAssignment = function(test,clickable){
		...
		if(orgId){
			data += "<a class='openInstAssignment' data-test-id='"+test.id+"' data-user-role='"+userRole+"'\n\
				href='/organization/"+orgId+"/assignment/"+userRole+"/"+test.id+"'>"+test.name+"</a>";
		}
    		return data;
	};
	...
	var getQuestionLink = function(ques,title,clickable){
		...
		var data = "";
		if(orgId){
			data += "<a class='openQuesPage' data-qid='"+ques.id+"' data-org-id='"+orgId+"'\n\
				href='/organization/"+orgId+"/question/"+ques.id+"'>"+title+"</a>";
		}else{
			data += "<a class='openQuesPage' data-qid='"+ques.id+"' href='/question/"+ques.id+"'>"+title+"</a>";
		}
    		return data;
	};
	...
	var getStatusFeed = function(feed,newsFeedId,clickable){
		...
		msg = tempDiv.text();
		tempDiv.remove();
		var html = "<a class='openStatusFeed' data-feed-id='"+feed.id+"' \n\
				href='/organization/"+orgId+"/feed/"+feed.id+"'>"+msg+"</a>";
    		return html;
	};
	...
	var getVideo = function(video,clickable){
		...
		if(orgId){
			data += "<a class='openInstVideo' data-id='"+video.id+"'\n\
				href='/organization/"+orgId+"/video/"+video.id+"'>"+video.name+"</a>";
		}else{
			data += "<a class='openVideoPage' data-id='"+video.id+"' href='/video/"+video.id+"'>"+video.name+"</a>";
		}
    		return data;
	};
	var getDoc = function(doc,clickable){
		...
		if(orgId){
			data += "<a class='openInstDoc' data-id='"+doc.id+"'\n\
				href='/organization/"+orgId+"/document/"+doc.id+"'>"+doc.name+"</a>";
		}else{
			data += "<a class='openDocPage' data-id='"+doc.id+"' href='/document/"+doc.id+"'>"+doc.name+"</a>";
		}
    		return data;
	};
	var getFile = function(doc,clickable){
		...
		if(orgId){
			data += "<a class='openInstFile' data-id='"+doc.id+"'\n\
				href='/organization/"+orgId+"/file/"+doc.id+"'>"+doc.name+"</a>";
		}else{
			data += "<a class='openFilePage' data-id='"+doc.id+"' href='/file/"+doc.id+"'>"+doc.name+"</a>";
		}
    		return data;
	};
        var getModule = function(doc,feed,clickable){
    		var orgId = $("#myInstitutePage").data('orgId');
		var data = i18nJS("ENTITY_MODULE")+" - ";
		var target = feed.info.target;
		if(orgId){
			if(target === null || target === undefined || target === ""){
				data += "<span data-id='"+doc.id+"'\n\
				title='Please go to the module section of library to open module.>"+doc.name+"</span>";
			}
			else{
				data += "<a class='openInstModule' data-id='"+doc.id+"'\n\
				href='/organization/"+orgId+"/module/"+doc.id+"?sectionId="+target.id+"' data-section-id='"+target.id+"'>"+doc.name+"</a>";
			}
		}else{
			data += "<span data-id='"+doc.id+"'\n\
				title='Please go to the module section of library to open module.>"+doc.name+"</span>";
	...
	var getUserLink = function(user,name){
		...
		if(user.isOrgUser){
			...
			a = "<a class='openInstProfile "+extraClass+"' href='/organization/"+user.orgId+"/profile/"+user.userId+"' \n\
				data-user-id='"+user.userId+"'>"+name+"</a>";
		}else{
			a = "<a class='openUserProfile' href='/user/"+user.userId+"' data-user-id='"+user.userId+"'>"+name+"</a>";
		}
		return a;
	};

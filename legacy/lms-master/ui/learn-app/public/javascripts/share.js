...
var shareUi = new function(){
    ...
    this.openInst = function(orgId,orgName,entityType,popup,showShareBtn,cbFn){
        ...
        $.get("Share/shareWithInst",params,function(data){
            swal({
                html:data,
                showConfirmButton:false
            });
            // $(".shareSelect").on("change",changeSelect);
                // popup.html(data);
            registerFns(popup);
            var shareWithDiv = $("#shareSection").find(".shareWithDiv").data("noSection",true);
            shareWithDiv.data("allowAllShare",true);
            var vChoose = $("#shareSection").find(".shareSelect");
    ...
    var shareToInst = function(shareDiv){
        // console.log("inside SHARETOInst");
    ...
    var registerFns = function(popup){
        $(".shareWithDiv").on("click",".instSelectShareCenter",centerClicked)
        ...
        $(".shareWithDiv").off("click",".shareEntityRetry",retryShare).on("click",".shareEntityRetry",retryShare);
        //disableShareBtn();
    };
    ...
    var getIndvUi = function(vChoose){
        ...
        smallLoader(holder);
        vReq.get("Share/getShareIndvUi",params,function(data){
    ...
    var getPublicUi = function(vChoose){
        ...
        var params = {'entityType':entityType};
        smallLoader(holder);
        vReq.get("Share/getSharePublicUi",params,function(data){
            holder.html(data).find(".entrySuggInput").focus();
            enableShareBtn();
        });
    };
    var getInstUi = function(vChoose){
        ...
        if(popupH.find(".shareWithDiv").data("allowAllShare")){
            ...
            var holder = shareDiv.find(".shareTypeHolder");
        // smallLoader(holder);
        vReq.get("Share/shareableBatches",params,function(data){
            $("#programShare").html(data);
            // holder.html(data);
    ...
    this.getBatchDetails = function(vChoose,programmeId,url){
        ...
        smallLoader(holder);
                url=url||"Share/shareableCenters";

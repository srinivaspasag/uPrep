...
TypeScript app (`uprep-ui`)** that delivers both the student LMS (`/learn/*`) and
...
implementation (`lms_nextgen-master`) — a modern Spring Boot 2.3 API-only rewrite
...
- **What's new (deployed today):**
  ...
  - **Replaced the entire legacy UI** with a brand-new **Next.js + React + TypeScript** app (`uprep-ui`).
    - Student **LMS** → `/learn/*`
    - Admin **CMDS** → `/cmds/*`
...
- **What's next (long-term direction):**
  - **`lms_nextgen-master`** = a modern **Spring Boot 2.3, API-only** rewrite of the backend (verified to build/run).
...
2. **Replaced the legacy Play 1.2.4 UI apps** (`web-app`, `learn-app`,
   `cmds-app`) with a **brand-new Next.js application** (`uprep-ui`) that talks to
...
flowchart TB
    ...
    subgraph AWS["AWS EC2 (m7i-flex.large, ap-south-1)"]
      ...
      subgraph UI["uprep-ui (Next.js, Node 20)"]
        P["React pages\n/learn/* (LMS)\n/cmds/* (CMDS)"]
...
- **Shared libs (`uprep-ui/lib/`):** `mongo.ts` (connection), `session.ts`
...
| Concern | Legacy (`lms-master`) | New (`uprep-ui` + deploy) | Why changed |
|---|---|---|---|
...
| `ui` | `node:20-bullseye` | Builds & serves `uprep-ui` on :3000 |
...
- New UI: `uprep-ui/` (pages `app/learn/*`, `app/cmds/*`; APIs `app/api/**/route.ts`; libs `lib/*`).
...
- Next-gen backend (WIP, Spring Boot): `lms_nextgen-master/`.

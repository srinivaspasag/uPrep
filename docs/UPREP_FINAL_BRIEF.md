...
- **Two stacks:** a modern API backend (`lms_nextgen-master`, deployable) and a legacy full app that holds the actual UI (`lms-master`, demo-only).
...
- **Modern backend** (`lms_nextgen-master`) — Spring Boot / Java 17, **8 services, ~400 APIs**. Container-ready and cloud-deployable. **But it has no user interface — APIs only.**
...
| Website (`uprep-website-master`) | PHP / Apache | **Yes** | Container or Amplify/S3+CloudFront |

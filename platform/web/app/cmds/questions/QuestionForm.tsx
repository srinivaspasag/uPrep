...
// Shared by both /cmds/questions/new (create, questionId undefined) and
// /cmds/questions/[id]/edit (edit, questionId set) — item #6 on the tracked
// issue list ("Question edit option") found there was no edit path at all,
// only creation. Rather than fork a second ~400-line form that would drift
// out of sync with this one, both routes render this same component.
export default function QuestionForm({ questionId }: { questionId?: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/cmds/questions/${questionId}`)
      .then((r) => r.json())
      .then((d) => {
        if (d.error) {
          setError(d.error);
        ...
        setParagraph(q.paragraph || "");
        setOptions(q.options && q.options.length ? q.options : ["", "", "", ""]);
        setCorrect(Array.isArray(q.correct) ? q.correct : []);
        setNumeric((q.numericAnswers || []).join(", "));
        setMatrixPairs((q.matrixPairs || []).join(", "));
        setSolution(q.solution || "");
        setDifficulty(q.difficulty || "EASY");
        setSubject(q.subject || "");
        setTags((q.tags || []).join(", "));
        setBoardIds(q.boardIds || []);
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const r = await fetch(isEdit ? `/api/cmds/questions/${questionId}` : "/api/cmds/questions", {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 text-slate-400">Loading…</div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        <h1 className="text-2xl font-semibold text-slate-800">{isEdit ? "Edit Question" : "Add Question"}</h1>
        ...
        {isEdit && published && (
          <p className="mt-3 rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-700 ring-1 ring-amber-200">
            This question is published — saving changes updates it immediately for future test attempts. Past
            attempts already graded are not retroactively changed.
          </p>
        )}
        ...
        <form onSubmit={submit} className="mt-6 space-y-6">
          ...
          <div className="rounded-xl bg-white p-5 ring-1 ring-black/5">
            ...
            <EquationToolbar textareaRef={questionRef} value={content} onChange={setContent} />
            <textarea
              ref={questionRef}
              value={content}
            ...
            <div className="mt-4">
              ...
              {subject && (
                <p className="mt-1 text-xs text-slate-400">
                  Subject: <span className="font-medium text-slate-600">{subject}</span> — derived from the
                  chapter(s) tagged above, so it can't get out of sync with the Board Tree.
                </p>
              )}
            </div>
          </div>
          ...
          {hasOptions(type) && (
            <div className="rounded-xl bg-white p-5 ring-1 ring-black/5">
              ...
              <p className="mt-2 text-xs text-slate-400">
                Symbols insert into whichever option you clicked into last (Option {activeOption + 1} now).
              </p>
              <EquationToolbar
                textareaRef={{ current: optionRefs.current[activeOption] ?? null }}
                value={options[activeOption] || ""}
                onChange={(v) => setOption(activeOption, v)}
              />
              <div className="mt-3 space-y-2">
                {options.map((opt, i) => (
                  <div key={i}>
                    ...
                    {opt.includes("$") && (
                      <div className="ml-7 mt-1 rounded bg-slate-50 px-2 py-1 text-sm text-slate-700">
          ...
          <div className="rounded-xl bg-white p-5 ring-1 ring-black/5">
            ...
            <EquationToolbar textareaRef={solutionRef} value={solution} onChange={setSolution} />
            <textarea
              ref={solutionRef}
              value={solution}

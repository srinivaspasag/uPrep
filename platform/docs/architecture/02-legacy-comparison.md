...
1. **Test retake behavior.** Assumed: retaking a test should just work and analytics should reflect all attempts. Actual legacy rule (`AnalyticsManager.isMultiAttemptAllowed()`, hardcoded `return false`): **one attempt per test, ever.** Legacy's UI never lets a student even reach the rejection — it checks status up front (`Tests.java testPageDirect` / `_isReAttemptTest()`) and routes straight to the existing result. The rebuild now does the same (`alreadyAttempted` flag + a read-back-only result endpoint).
...
## 6. Net new capability inventory (does not exist in legacy at all)
...
## 7. What still fails if legacy goes down

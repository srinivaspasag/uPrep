...
- `website` (PHP website/UI)
- `services/virtual-classroom` (Spring Boot)
- `backend` (core microservices for current backend)
...
### 3.3 Codebase architecture inside `backend`

`backend` is organized by domains with a repeatable pattern:
...
```bash
cd "$WORKSPACE/services/virtual-classroom"
mvn -Dmaven.repo.local="$M2_LOCAL" -DskipTests clean package
```

#### B) `backend` full domain build
...
```bash
php -S localhost:8000 -t "$WORKSPACE/website"
```
...
This part is the result of a full source review of `backend` (the active backend). It documents the real API surface, backend design, database design, a back-of-envelope scaling analysis, and concrete limitations/constraints found in code.

## B1) Architecture (Detailed, As-Built)

### B1.1 Deployment units

`backend` is a Maven multi-module build that produces **independent Spring Boot fat-jars per domain**. There are 8 runnable domain services plus shared libraries:
...
- **One image, eight services.** All `lms_nextgen` services build from a single image
  (`backend/Dockerfile`, multi-stage Maven → JRE). Each container selects its
